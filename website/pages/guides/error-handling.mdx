...
description: Every way a thumbnail can fail, and the [CODE] trick that turns a native string into a typed .code.
---
...
<Callout type="info">
Every way a thumbnail can fail, and the small trick that turns an opaque native string into a typed `.code` you can `switch` on.
</Callout>
...
[`toThumbnailError`](https://github.com/pythonsst/react-native-nitro-thumbnail/blob/main/src/errors.ts) reverses it, with a small ladder of

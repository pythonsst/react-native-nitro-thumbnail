---
title: Issues Solved
description: How react-native-nitro-thumbnail addresses the common, reported failure modes of older video-thumbnail libraries.
---
...
| `package org.apache.commons.io … does not exist` / missing `LastModifiedFileComparator` | #29, #31 | **No Apache Commons dependency.** LRU eviction is a tiny pure-Kotlin helper (`filesToEvict`), unit-tested. |
...
| `setDataSource failed` for YouTube links | #62 | A YouTube *page* URL isn't a direct media stream — no library can decode it. We fail cleanly and document that `url` must point at an actual video file/stream. |
...
| "File doesn't exist or not supported" for a valid URI | #26 | Clear separation: a missing local file is `FILE_NOT_FOUND`; an unreadable/undecodable one is `DECODE_FAILED`. |
...
| Reported problem | Issues | How nitro-thumbnail handles it |
| --- | --- | --- |
| `AVFoundationErrorDomain` -11832 / -11800 "cannot be used" crashes | #56, #117 | Frame extraction is wrapped; an undecodable asset becomes a typed `DECODE_FAILED` (or `REMOTE_FETCH_FAILED` for network) — not an unhandled native error. |
...
| Configurable JPEG/PNG **quality** | #148 | First-class `quality` option (`0..1`) from day one. See [API Reference](/guides/api-reference). |
| Precise frame **time tolerance** | #142 | First-class `timeToleranceMs` option (set `0` for an exact seek). |
| Opaque `Error: null` | #16 | Every failure is a typed `ThumbnailError` with a `.code` — never a silent `null`. See [Error Handling](/guides/error-handling). |
...
- **DRM-protected** media can't be decoded by `AVFoundation` / `MediaMetadataRetriever`.

When something genuinely can't be done, you get a typed
[`ThumbnailError`](/guides/error-handling) — not a crash.

## Summary

| Theme | Status |
| --- | --- |
| Android build/compile breakage | ✅ Kotlin + modern AGP, no jcenter, no Apache Commons |
...
Switching over? See the [Migration guide](/guides/migration) — it's usually a

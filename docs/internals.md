...
linting), see [`CONTRIBUTING.md`](../CONTRIBUTING.md). This document is the
...
[architecture](./architecture.md). The principle throughout: **extract the pure
logic so it can be tested without a device.**

| Layer | What's tested | How | Where |
|---|---|---|---|
| TS public API | validation, defaults, error mapping, web pipeline | Jest (node + jsdom) | [`__tests__/`](../__tests__) |
...
| End-to-end | real decode on real media | example app | [`example/`](../example) |
...
1. Add it to `CreateThumbnailOptions` in [`src/types.ts`](../src/types.ts) and to
   `NativeThumbnailOptions` in [`src/specs/Thumbnail.nitro.ts`](../src/specs/Thumbnail.nitro.ts).
2. Normalize/default it in [`src/index.ts`](../src/index.ts) (and `index.web.ts`).
...
1. Add it to the `ThumbnailErrorCode` union *and* `THUMBNAIL_ERROR_CODES` array in
   [`src/types.ts`](../src/types.ts) (both — the array is the runtime allow-list
...
4. Document it in [`docs/error-handling.md`](./error-handling.md) and the
...
- **Validate in TypeScript, act dumbly in native.** Don't push input validation
  ...
  struct. (See [architecture → design principles](./architecture.md#design-principles).)
...
- [ ] Verified in the [example app](../example) if native code changed
- [ ] Docs updated ([API reference](./api-reference.md) / relevant guide)
- [ ] Small, focused PR — discuss API/implementation changes in an issue first

Friendly reminder: the [code of conduct](../CODE_OF_CONDUCT.md) applies to all

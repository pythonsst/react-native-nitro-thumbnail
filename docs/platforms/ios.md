...
> Pure Swift, AVFoundation, ImageIO. No Objective-C glue, no bridge.

**Source:** [`ios/HybridThumbnail.swift`](../../ios/HybridThumbnail.swift) ·
[`ios/ThumbnailEncoder.swift`](../../ios/ThumbnailEncoder.swift)
...
`DECODE_FAILED`. See [error handling](../error-handling.md) for how the `[CODE]`
prefix reaches JS.
...
- `enforceLimit` lists the directory, gathers `(size, modificationDate)` for each
  ...
  `ThumbnailEncoder.filesToEvict`. See [caching](../caching.md).
...
- The pod is declared in [`NitroThumbnail.podspec`](../../NitroThumbnail.podspec);
  `ios/Tests/**` is excluded from the shipped pod.
- Pure-Swift helpers (`ThumbnailEncoder`) are covered by XCTest under `ios/Tests`.
- End-to-end verification runs through the [`example/`](../../example) app on a
  simulator/device.
